...
- **图形版开机自启崩溃**：在图形版里开启「开机自启」后，重启会弹
  「'NoneType' object has no attribute 'write'」崩溃框。两个原因叠加：
  自启命令误用了源码模式的 `-m campusnet` 参数（对 exe 是非法命令）；
  无窗口进程的 stderr 是 `None`，argparse 报错时写不出去。现在
  打包版的自启命令不再带 `-m`，入口处把 `None` 输出流兜底成 devnull。
- **双击图标即可自动连网**：已配置好账号的情况下，打开程序检测到没联网
  会自动尝试连接一次（只试一次，不循环重试；失败可在日志里看原因，
  或点「立即重新认证」手动再来）。
